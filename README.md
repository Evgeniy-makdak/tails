...
| **Геозоны** | Создание, **редактирование** и **удаление** безопасных/опасных зон; звук при выходе / входе |
...
- Список зон: тап по карточке → **изменить на карте** (размер, место, тип, название) или **удалить** с подтверждением. Зоны хранятся локально в аккаунте (AsyncStorage).
